...
export default function Home() {
  ...
  const handleSearch = () => {
    ...
    const exactMatch =
      zones.find((zone) => normalizeText(zone.name) === normalizedQuery) ?? null;
    ...
    const matchedZone = exactMatch ?? partialMatch;
    ...
    setSelectedChip(matchedZone.name);
    setSearchText(matchedZone.name);
  };
  ...
  const handleChipPress = (chip: string) => {
    ...
    const matchedZone =
      zones.find((zone) => normalizeText(zone.name) === normalizeText(chip)) ??
      zones.find((zone) =>
        normalizeText(zone.name).includes(normalizeText(chip)),
      ) ??
      null;

    setSelectedZoneId(matchedZone?.id ?? null);
  };
